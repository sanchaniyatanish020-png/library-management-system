...
const MyBorrows = () => {
    ...
    const handleReturn = async (id) => {
        try {
            ...
            showMessage('Book returned! Fine: Rs.' + res.data.fine, 'success')
            fetchBorrows()
        } catch (err) {
    ...
    const handleReceipt = async (id) => {
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/pdf/borrow/${id}`, {
                headers: {
                                    ...
                                    return (
                                        <tr key={b.id} className="border-t border-slate-50 hover:bg-slate-50">
                                            ...
                                            <td className="px-5 py-3 text-xs font-semibold"
                                                style={{ color: getFineColor(b.fine) }}>
                                                {getFineText(b.fine)}
                                            </td>
                                            <td className="px-5 py-3">
